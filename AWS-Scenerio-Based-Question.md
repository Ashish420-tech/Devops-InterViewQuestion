...
Cache frequently accessed data using ElastiCache (Redis).
Store static content in S3 and distribute it through CloudFront.
Use Amazon RDS Read Replicas for read-heavy workloads.
Decouple components using Amazon SQS.
Monitor using CloudWatch and configure scaling policies.
Perform load testing before the event.
2. How do you achieve Disaster Recovery (DR) for your cloud application?
...
Replicate S3 data using Cross-Region Replication (CRR).
Use Route 53 Failover Routing.
Store infrastructure as code using Terraform or CloudFormation.
Automate recovery with CI/CD pipelines.
...
Deploy resources inside private subnets.
...
"I designed a highly available three-tier architecture consisting of:

Public Subnets for the Application Load Balancer and NAT Gateway.
Private Subnets for EC2 application servers managed by Auto Scaling Groups.
Database Subnets for Amazon RDS Multi-AZ.
CloudFront serving content from Amazon S3.
CloudWatch for monitoring and alarms.
CI/CD using GitHub Actions/Jenkins with Terraform for infrastructure provisioning."
5. What was the biggest challenge during cloud architecture design?

Answer:

Balancing high availability with cost optimization.
Designing secure network segmentation.
Preventing single points of failure.
Selecting the right scaling strategy.
Managing cross-region disaster recovery.
Reducing latency while maintaining security.
6. How do you choose one AWS service over another?
...
EC2 for long-running workloads.
7. Difference between SQL and NoSQL Databases?
SQL	NoSQL
Relational	Non-relational
Fixed Schema	Flexible Schema
ACID Transactions	BASE/Eventual Consistency (depending on the database)
Vertical Scaling	Horizontal Scaling
Complex Joins	No Joins
MySQL, PostgreSQL	DynamoDB, MongoDB, Cassandra
8. What is Cloud Computing?

Answer:

Cloud computing is the on-demand delivery of computing resources such as servers, storage, databases, networking, and software over the internet with pay-as-you-go pricing, enabling scalability, flexibility, and reduced operational overhead.

9. What is AWS Service X?

Answer Framework (for any AWS service):

When asked about any AWS service, answer in this order:
